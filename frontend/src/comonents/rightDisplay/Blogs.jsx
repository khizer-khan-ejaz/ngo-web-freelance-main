...
const Blogs = () => {
  ...
  // Control how many blogs to display
  const [displayCount, setDisplayCount] = useState(5); // Number of blogs to show
  ...
  const handleDelete = async (e, blog) => {
    ...
    try {
      ...
      setCollapseState(prev => data.events.map(() => false));
    } catch (err) {
      console.log(err.message);
    }
  };
  ...
  return (
    <div className='scroller w-[100%] p-2 relative h-[100%] gap-[20px] overflow-scroll'>
        ...
        <Flex transition="all 0.3s ease-in-out" w={{ base: "70%", md: "100%" }} mt={"20px"} flexWrap={"wrap"} gap={"20px"} justifyContent={"center"} alignItems={"center"}>
          {blogs.slice(0, displayCount).map((blog, index) => (
            <div key={index} className='bg-white transition-all duration-500 w-[100%] flex flex-col justify-center items-center max-w-[300px] shadow-md'>
              ...
              <div className='transition-all duration-500 w-[100%] flex justify-center items-center h-[150px] md:h-[190px] lg:max-h-[220px]'>
      ...
      {displayCount < blogs.length && (
        <Button onClick={() => setDisplayCount(prev => prev + 5)} mt={4}>
          Load More
        </Button>
      )}
    </div>
